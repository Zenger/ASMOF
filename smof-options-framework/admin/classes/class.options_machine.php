...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
			 	...
			 	$_small = "";
			 	if ($value['type'] == "retina") 
			 	{
			 		$_small = " / <small>" . (isset($value['desc']) ? $value['desc'] : "") . "</small>";
			 	}
				...
				if($value['name']) $output .= '<h3 class="heading">'. $value['name'] . $_small .'</h3>'."\n";
				
				$output .= '<div class="option">'."\n" . '<div class="controls '.$mtype.' ">'."\n";
			...
			switch ( $value['type'] ) {
				...
				//text input
				case 'text':
					$data[$value['id']] = (!isset($data[$value['id']])) ? $defaults[$value['id']] : $data[$value['id']];
				...
				case 'select':
					...
					$mini ='';
					if ( ! isset($value['options']) ) break;
					#if ( !isset($data[$value['id']]) ) break;
					$data[ $value['id'] ] = (!isset($data[ $value['id'] ])) ? "of-" . rand() : $data[$value['id']];
					$value['id'] = (!isset($data[$value['id']])) ? "of-" . rand() : $value['id'];
				...
				case 'retina':
					$data[$value['id']] = (!isset($data[$value['id']])) ? $defaults[$value['id']] : $data[$value['id']];
					if(!isset($value['mod'])) $value['mod'] = '';
					$output .= Options_Machine::optionsframework_retina_uploader_function($value['id'],$value['std'],$value['mod']);			
				break;
				...
				// typography with google web fonts
				case 'webfont_typography':
					#$data[$value['id']] = (!isset($data[$value['id']])) ? $defaults[$value['id']] : $data[$value['id']];
					$typography_stored = isset($data[$value['id']]) ? $data[$value['id']] : $value['std'];
					...
					if(isset($typography_stored['size'])) {
						$output .= '<div class="select_wrapper typography-size" original-title="Font size">';
						$output .= '<select class="of-typography of-typography-size select wf-size" name="'.$value['id'].'[size]" id="'. $value['id'].'_size">';
							for ($i = 9; $i < 41; $i++){ 
								$test = $i.'px';
								$output .= '<option value="'. $i .'px" ' . selected($typography_stored['size'], $test, false) . '>'. $i .'px</option>'; 
								}
					...
					if(isset($typography_stored['face'])) {
						...
						$output .= '<div class="select_wrapper typography-face" original-title="Font family">';
						$output .= '<select class="of-typography of-typography-face select google_font_select" name="'.$value['id'].'[face]" id="'. $value['id'].'_face">';
						
						$faces = array( 'custom' => 'Custom Font',
										'arial'=>'Arial',
										'verdana'=>'Verdana, Geneva',
										'trebuchet'=>'Trebuchet',
										'georgia' =>'Georgia',
						...
						$faces = array_merge($faces, $wf->getList());
						foreach ($faces as $i=>$face) {
							$output .= '<option value="'. $i .'" ' . selected($typography_stored['face'], $i, false) . '>'. $face .'</option>';
						}			
					...
					if(isset($typography_stored['style'])) {
						...
						$output .= '<div class="select_wrapper typography-style" original-title="Font style">';
						$output .= '<select class="of-typography wf-style of-typography-style select" name="'.$value['id'].'[style]" id="'. $value['id'].'_style">';
						$styles = array('normal'=>'Normal',
										'italic'=>'Italic',
										'bold'=>'Bold',
										'bold italic'=>'Bold Italic');
					...
					if (!isset($typography_stored['style'])) { 
						$typography_stored['style'] = null;
					}

					if (!isset($typography_stored['color']))
					{
						$typography_stored['color'] = null;
					}

					if (!isset($typography_stored['face']))
					{
						$typography_stored['face'] = null;
					}

					if (!isset($typography_stored['height']))
					{
						$typography_stored['height'] = null;
					}
					...
					switch ($typography_stored['style']) {
						case 'bold italic':
					...
					$style .= "font-size:" . $typography_stored['size'] . ";";
					$style .= "font-family: ". $typography_stored['face'] . ";";
					$style .= "color:" . $typography_stored['color'] . ";";
					$style .= "'";
					...
					$output .= '<p class="'.$value['id'].'_face_ggf_previewer webfont_typography_preview webfont_typography_preview" '. $style .'>'. $g_text .'</p>';
			...
			if ( $value['type'] != 'heading') { 
				...
				$default_text = '';
				if ($value['type'] != "images" && $value['type'] != "image" && $value['type'] != "upload" && $value['type'] != "info" && $value['type'] != "media" && isset($value['std']) && !is_array($value['std']) && $value['std'] !== "")
				{
					if ($value['type'] == "switch")
					{
						$dfval = ($value['std'] == true) ? "On" : "Off";
					}
					elseif ($value['type'] == "checkbox")
					{
						$dfval = ($value['std'] == true) ? "On" : "Off";
					}
					elseif ($value['type'] == "multicheck" || $value['type'] == "select" || $value['type'] == "radio")
					{
						$dfval = $value['options'][$value['std']];
					}
					else
					{
						$dfval = $value['std'];
					...
					$default_text = (!isset($value['desc']) || empty($value['desc'])) ? "" : "<br />";
					$default_text .= '<strong>Default value: "'.$dfval.'"' . "</strong>";
				}

				if(!isset($value['desc'])){ $explain_value = ''; } else{ 
					$explain_value = '<div class="explain">'. $value['desc'] .' '.$default_text.'</div>'."\n"; 
				} 

				if ($value['type'] == "retina") $explain_value = "";
	...
	public static function optionsframework_retina_uploader_function($id,$std,$mod){
		...
		$uploader .= '<div> <input class="'.$hide.' alignleft upload of-input" name="'. $id .'" id="'. $id .'_upload" placeholder="Regular sized image" value="'. $val .'" /> <span class="explain alignleft" style="margin:8px 0 0 8px;">Regular sized image URL</span>  </div>';
		$uploader .= '<div> <input class="'.$hide.' alignleft upload of-input" name="'. $id .'2x" id="'. $id .'_upload" placeholder="Retina sized image" value="'. $retina .'" /> <span class="explain alignleft" style="margin:8px 0 0 8px;">Retina sized image URL</span>  </div>';
